...
const ArrowLink = ({ children, link, type, content, target, noprefix }) => {
  function ArrowType() {
    const href = link || "#";
    if (type == "back") {
      return (
        ...
        >
          ...
          {!noprefix && <span className={styles.Text}>Previous: </span>}
      ...
      return (
        <Link
          ...
          target={target == "_blank" ? target : "_self"}
        >
          {!noprefix && <span className={styles.Text}>Next: </span>}
          <span className={styles.Truncate}>{content}</span>
          <svg
            width="14"
            height="14"
